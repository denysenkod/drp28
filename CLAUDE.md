...
- All other routes → serve the Next.js static export from `/out/` via the ASSETS binding

### Frontend
- **app/**: Next.js App Router pages (home, questionnaire, search, profile, shared profile)
- **components/**: Modular React UI, including shadcn-style primitives in `components/ui/`
- **lib/**: API clients, local storage, quiz logic, brief/profile helpers, and shared types
- **public/Images/**: Static image assets copied from the legacy frontend image folder
- **out/**: Generated by `npm run build`; served by Cloudflare Workers assets
...
### Local Development
- **Frontend dev**: `npm run dev` launches Next.js
- **Worker simulation**: `npm start` builds the Next export and launches Wrangler dev (includes D1 local binding)
- **Without npm after `out/` exists**: `node local-dev.mjs` (Node.js fallback server; memory-only storage, no persistence between restarts)
...
### wrangler.toml
- Worker name: `drp28`
- Assets binding: `ASSETS` → serves `/out/` as static files with SPA fallback
...
### Build output
`npm run build` exports the Next.js frontend to `out/`. Run it before Wrangler dev/deploy or before using `node local-dev.mjs` as a static server for the new UI.
...
- Frontend is Next.js + React + Tailwind CSS, exported statically for the Worker assets binding.
