...
test('new static frontend is wired to image and database APIs', async () => {
  ...
  const profile = await readFile(new URL('../components/profile-builder.tsx', import.meta.url), 'utf8');
  const brief = await readFile(new URL('../lib/brief.ts', import.meta.url), 'utf8');
  const shared = await readFile(new URL('../components/shared-profile.tsx', import.meta.url), 'utf8');
  ...
  assert.ok(api.includes('"/api/gallery"'));
  assert.ok(api.includes('"/api/favorites"'));
  assert.ok(api.includes('"/api/user-photos"'));
  assert.ok(api.includes('"/api/briefs"'));
  assert.ok(profile.includes('Complete your profile'));
  assert.ok(profile.includes('Is there anything else you would like to tell your barber?'));
  assert.ok(profile.includes('Every saved haircut appears here automatically'));
  assert.ok(profile.includes('item.source === "upload" ? "Uploaded"'));
  assert.ok(!profile.includes('Share with stylist'));
  assert.ok(!profile.includes('hairstyleStatus'));
  assert.ok(brief.includes('mergeReferences'));
  assert.ok(brief.includes('styleToReference(style, "favourite")'));
  assert.ok(shared.includes('useSearchParams'));
  assert.ok(shared.includes('getBrief(id)'));
  assert.ok(nextConfig.includes('output: "export"'));
  assert.ok(tailwindConfig.includes('./app/**/*.{ts,tsx}'));
});
...
test('wrangler deploys the TypeScript worker entry', async () => {
  ...
  assert.match(config, /^directory = "\.\/out"$/m);
